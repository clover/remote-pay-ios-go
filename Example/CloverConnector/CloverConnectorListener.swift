        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if authResponse.success {
                    ...
                    if let payment = authResponse.payment,
                        ...
                        if authResponse.isSale || authResponse.isAuth {
                            ...
                            if authResponse.isSale {
                                ...
                                if expectedResponseId {
                                    strongSelf.showMessage("Auth successfully processed", duration: 5)  // Happy Path
                                } else {
                ...
                if authResponse.result == .CANCEL {
                    ...
                    strongSelf.showMessage("Auth Tx Failed - \(String(describing: authResponse.message))", duration: 5)
                } else {
...
//                            store.addPaymentToOrder(posPayment, order: order)
                ...
                if preAuthResponse.result == .CANCEL {
                    ...
                    strongSelf.showMessage("PreAuth Tx Failed - \(String(describing: preAuthResponse.message))", duration: 5)
                } else {
    ...
    public func  onTipAdjustAuthResponse ( _ response:TipAdjustAuthResponse ) -> Void {
        DispatchQueue.main.async { [weak self] in
            ...
            if response.success {
                ...
                strongSelf.showMessage("Tip adjust failed - \(String(describing: response.message))", duration: 5)
            }
        }
    }
    ...
    public func  onVoidPaymentResponse ( _ response:VoidPaymentResponse ) -> Void {
        DispatchQueue.main.async { [weak self] in
            ...
            if response.success {
                ...
                strongSelf.showMessage("There was an error voiding payment: " + String(describing: response.message))
            }
        }
    }
    ...
    public func  onCloseoutResponse ( _ response:CloseoutResponse ) -> Void {
        DispatchQueue.main.async { [weak self] in
            if response.success {
                ...
                self?.showMessage("Error scheduling closeout: " + (response.message ?? ""), duration: 5)
            }
        }
    }
    ...
    public func onRefundPaymentResponse(_ refundPaymentResponse: RefundPaymentResponse) {
        DispatchQueue.main.async { [weak self] in
            ...
            if refundPaymentResponse.success {
                ...
                strongSelf.showMessage("Refund failed. \(String(describing: refundPaymentResponse.message))", duration: 5)
            }
        }
    }
